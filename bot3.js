// botv2.js (fixed: avoid setRawMode EIO in child / non-TTY environments)
// Based on your original file (kept all core functions intact).
// If you run in parallel mode (fork), child processes will not create readline.
...
// ==========================================
// Detect child / non-tty (Termux-safe)
// If MULTI_CHILD set OR stdin is not a TTY, treat this as a child/non-interactive run
// ==========================================
...
// We'll wrap console.log per-run inside startScript for per-run counting
// ==========================================
// SPINNER HELPERS
// ==========================================
...
// ==========================================
// READLINE: create only when interactive parent
// ==========================================
let rl = null;
if (!isChildProcess) {
...
// ==========================================
// ENTRY: Mode selection & multi-wallet prompt (only in parent / interactive mode)
// If non-interactive (child), we'll skip this and run startScript directly later.
// ==========================================
if (!isChildProcess) {
      ...
      if (["y", "yes"].includes(multiAns.toLowerCase())) {
        ...
        if (mwMode === "2") {
          ...
          for (let i = 0; i < wallets.length; i++) {
            ...
            // spawn child process with PRIVATE_KEY env set and MULTI_CHILD flag
...
async function startScript(opts = {}) {
  ...
  // start watching (this function keeps running unless terminated)
...
// ==========================================
// If this process is a child (forked / non-interactive), start immediately
// (We call startScript() here — startScript is hoisted because it's a function declaration)
// ==========================================
if (isChildProcess) {
  // Run without interactive prompts
  (async () => {
    try {
      // Ensure PRIVATE_KEY exists in env for child
      ...
      // child should keep running (watcher) — do not exit immediately
    } catch (err) {
