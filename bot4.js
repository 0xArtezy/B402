// bot3_final.js - Multi-wallet (A: same MINT_COUNT for all wallets) + Termux-safe forks
// All core functions preserved. Only input & forking logic adjusted.
...
// Detect non-interactive / child process (avoid readline / setRawMode EIO)
const isChildProcess = process.env.MULTI_CHILD === "1" || (typeof process.stdin.isTTY === "boolean" && !process.stdin.isTTY);

// hide dotenv noisy message
...
// Readline only in parent interactive mode
let rl = null;
if (!isChildProcess) {
  rl = readline.createInterface({ input: process.stdin, output: process.stdout });
}
function ask(q) {
  if (!rl) throw new Error("Interactive prompt unavailable in non-TTY / child process.");
  return new Promise((res) => rl.question(q, (ans) => res(ans.trim())));
}

// ENTRY (parent only). If child process, skip to startScript below.
if (!isChildProcess) {
  (async () => {
    try {
      console.clear();
      ...
      if (["y","yes"].includes(multiAns.toLowerCase())) {
        ...
        // Ask mint count that applies to all wallets
        let mintMulti = await ask("Masukkan jumlah mint untuk semua wallet (default 10): ");
        if (!mintMulti || !mintMulti.toString().trim()) mintMulti = "10";
        // set into env for children and sequential runs
        process.env.MINT_COUNT = mintMulti.toString();
        ...
        if (mwMode === "2") {
          // Parallel mode: fork this script for each wallet
          ...
          for (let i = 0; i < wallets.length; i++) {
            ...
            // spawn child process with PRIVATE_KEY env and MULTI_CHILD flag and MINT_COUNT
            const child = fork(__filename, [], {
              env: { ...process.env, PRIVATE_KEY: pk, MULTI_CHILD: "1", MINT_COUNT: process.env.MINT_COUNT },
              stdio: "inherit"
            });
...
// ------------------------------------------
// startScript (main bot flow). Core functions kept intact.
// ------------------------------------------
async function startScript(opts = {}) {
  ...
  // per-run console wrapper to count formatted mint outputs
  ...
  console.log = function (...args) {
    ...
    // Avoid duplicate global summary prints
  ...
  // runClaim (unchanged semantics)
  async function runClaim(jwt) {
    console.log("🔍 Fetching payment requirement...");
    let pay;
    try {
      await axios.post(`${API_BASE}/faucet/drip`, { recipientAddress: RECIPIENT }, { headers: { Authorization: `Bearer ${jwt}` } });
    } catch (err) {
    ...
    function mintPermit(p, i) {
      return axios.post(`${API_BASE}/faucet/drip`, {
        recipientAddress: RECIPIENT,
        paymentPayload: { token: TOKEN, payload: p },
        paymentRequirements: { network: pay.network, relayerContract: pay.relayerContract }
  ...
  // watcher (spinner integrated) - unchanged
  const WATCH_ADDR = [
  ...
  // BOOT: spinners for captcha & challenge
  ...
  // start watching (keeps running)
  await watchDistribution(jwt);
} // end startScript

// If we are a child process (forked / non-interactive), run immediately
if (isChildProcess) {
  (async () => {
    try {
      if (!process.env.PRIVATE_KEY || !process.env.PRIVATE_KEY.startsWith("0x")) {
        console.error("Child process started without PRIVATE_KEY in env. Exiting.");
        process.exit(1);
      }
      // Ensure MINT_COUNT is present in env (parent set it)
      process.env.MINT_COUNT = process.env.MINT_COUNT || "10";
      await startScript({ isChild: true });
    } catch (err) {
