// ==========================================
// INPUT PRIVATE KEY & MINT COUNT (DEFAULT 10)
// ==========================================
...
// ───── 1-line SUMMARY BASED ON MINT LOGS ─────
...
console.log = function (...args) {
  ...
  // Sembunyikan summary asli
  if (txt.includes("📊 SUMMARY")) return;

  // Jika log menunjukkan selesai proses → tampilkan ringkasan 1 baris
  if (txt.includes("All permits processed") || txt.includes("All mints failed")) {
    originalLog5(`📊 SUMMARY: ${mintSuccess} success | ${mintFailed} failed`);
  }

  // tampilkan log normal
  originalLog5.apply(console, args);
};
...
(async () => {
  const INPUT_PK = await ask("Masukkan PRIVATE KEY: ");
  if (!INPUT_PK || !INPUT_PK.startsWith("0x")) {
  ...
  if (!INPUT_MINT.trim()) {
    ...
    INPUT_MINT = parseInt(INPUT_MINT);
    if (isNaN(INPUT_MINT) || INPUT_MINT <= 0) {
      console.log("❌ MINT_COUNT tidak valid!");
      process.exit(1);
    }
  }
  ...
  // inject ke env
  process.env.PRIVATE_KEY = INPUT_PK.trim();
  process.env.MINT_COUNT = INPUT_MINT.toString();
...
// ==========================================
// SELURUH SCRIPT ASLI DIMASUKKAN DI SINI
// ==========================================
